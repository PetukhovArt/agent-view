...
- `act` — a step protocol for a cheap model to drive a scenario ([ADR 0003](./docs/adr/0003-act-step-protocol-and-replay.md)). `act start --until-testid <id>` (or `--until-selector`) prints a numbered table of visible controls; `act click|type|select|drag <n> …` acts on a row, settles, checks the done condition and prints DONE or the next table in one call. `act scroll up|down` and `act wait` do the same without a row. `act do "…" "…"` runs several steps decided from one table. Rows are re-mapped when the page re-renders and refused when covered; a window reload under a step (login) is ridden over. `act save` / `start --save` records the run by test id or role + name; `act replay <name>` re-runs it inside the server with no model: exit 0 DONE, 1 FAIL (the app did not answer, or a control stayed disabled or covered), 3 STALE (a control is gone — re-record), 2 when it could not run. Passwords are masked in the table and never stored; replay reads `AGENT_VIEW_SECRET`.
- `AGENT_VIEW_SERVER_PORT` runs the server on another port, with its own token file, so a dev build can run beside the installed one.
- `click` and `fill` refuse more than one way of addressing the element (`<ref>`, `--filter`, `--testid`, `--selector`, `--pos`). Before, one silently won, and `fill 12 "abc" --testid x` typed `12`. `wait` takes exactly one of `--filter` / `--testid` / `--selector`, and `screenshot` at most one of `--crop` / `--testid` / `--selector`.
...
### Changed
- Scaled screenshots are PNG, not WebP/JPEG. Vision tokens depend on pixel size, not file size.
...
- New flags: `--cancel` (end with `dragCancel`), `--html5` (fail instead of falling back), `--pointer` (never intercept), `--mask <n>` (override `dragOperationsMask`, for `effectAllowed`/`dropEffect` diagnostics).
- `drag` now hovers the source before pressing, on both paths.
