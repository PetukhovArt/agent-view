...
- `act` — a step protocol for a cheap model to drive a scenario ([ADR 0003](./docs/adr/0003-act-step-protocol-and-replay.md)). `act start --until-testid <id>` prints a numbered table of visible controls; `act click|type|select|drag|scroll|wait <n> …` acts on a row, settles, checks the done condition and prints DONE or the next table in one call. `act do "…" "…"` runs several steps decided from one table. Rows are re-mapped when the page re-renders and refused when covered. `act save` / `start --save` records the run by test id or role + name; `act replay <name>` re-runs it inside the server with no model, exit 0 DONE, 1 FAIL, 3 STALE. Passwords are masked in the table and never stored; replay reads `AGENT_VIEW_SECRET`.
